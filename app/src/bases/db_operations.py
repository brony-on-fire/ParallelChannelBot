...
class ChatMessage:
    ...
    def check_permission_for_posting(self):
        ...
        if check_ban is not None:
            ...
            mute_end_date = check_ban_status.mute_end_date
...
class ChannelMessage:
    ...
    def connect_chat(self, linked_chat_id):
        ...
        if self.get_channel is None:
            new_channel = Channel(id_telegram = self.chat_id, linked_chat_id = self.linked_chat_id,
                                mute_timer = 3, votes_for_block = 10,
                                created_at = datetime.now(), updated_at = datetime.now())
            s.add(new_channel)
            s.commit()
        else:
            self.get_channel.update({Channel.linked_chat_id: self.linked_chat_id, Channel.updated_at: datetime.now()})
            s.commit()
...
class ChannelSettings(DBOperation):
    '''
    Класс для настройки связи с каналом
    '''
    def __init__(self, message, linked_chat_id):
        self.message_id = message.message_id
        self.chat_id = message.chat.id
...
class PostMessage(DBOperation):
    '''
    Класс для отправки собщения
    '''
    def check_permission_for_posting(self):
        '''
        Проверяет, что можно постить в канал
