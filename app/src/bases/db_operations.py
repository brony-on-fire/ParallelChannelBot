...
from .tables import engine_settings, Base, Channel, Post, Complain, Blacklist
...
class ChatPosting(ChatMessage):
    ...
    def check_permission_for_posting(self):
        ...
        check_ban = s.query(Blacklist).filter((Blacklist.id_telegram == self.user_id) & (Blacklist.channel_id == channel_id)).one_or_none()
...
class ComplainAction(ChannelMessage):
    ...
    def __init__(self, message, author):
        ...
        self.author = author
        self.get_complain = s.query(Complain).filter((Complain.channel_id == self.chat_id)
    ...
    def count_of_complains(self):
        ...
        if self.get_complain:
            ...
            complains_count = s.query(Complain).filter((Complain.post_id == post_id)
    ...
    def block_author(self):
        ...
        get_post = s.query(Post).filter((Post.channel_id == self.chat_id) & (Post.id_telegram == self.message_id)).one()

        #Сохраняем id автора поста в переменную
        post_author = get_post.author_id

        #Проверяем, что пользователь ранее не блокировался
        check_ban = s.query(Blacklist).filter((Blacklist.id_telegram == post_author) & (Blacklist.channel_id == self.chat_id)).one_or_none()
